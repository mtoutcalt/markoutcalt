...
export const GET: APIRoute = async () => {
	...
	const body = [
		...
		'## How to fetch it',
		'',
		'- Every post has a Markdown twin at the same path with `.md` appended —',
		'  the prose without the nav, scripts or layout markup.',
		`- \`curl ${SITE_URL}/blog/art-unix-design-rules.md\``,
		'- Both representations are static files; no `Accept` header is needed.',
		...
		...posts.flatMap((post) => [
			...
			`  - Markdown: ${new URL(`/blog/${post.id}.md`, SITE_URL).href}`,
		]),
		'',
		'## Machine-readable',
		'',
